        ...
        for character in line:
            ...
            # Check if character is code character
                ...
                image_character = characterTemplate(
                    character, character_color, character_shadow_color, character_bold, character_italic, unicode, character_random, character_underline, character_strike)
        ...
        if not line == []:
            for character in line:
                ...
                if character.character == " ":
                    ...
                    size = fnt.getsize(character.character)[0]
                    x = size + x
            if x > width:
                width = x + 10
            x = 8
...
def draw_italics(char, x, y, width, height, background, fnt, color):
    """
    Draws charcters on new image, shift different parts of characters to italcize it, then take new image and imprint it on the old image
    """
            ...
            for character in line:
                ...
                if character.random is True:
                    if not character.character == " ":
                        ...
                        magic = "!#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz{|}~⌂ÇüéâäàåçêëèïîìÄÅÉæÆôöòûùÿÖÜø£Ø×ƒáíóúñÑªº¿®¬½¼¡«»"
                ...
                if character.strike is True:
                    if line_number == 1:
                        y_line = 16
                        draw.line((x+2, y_line, x + 2 + fnt.getsize(character.original)
                                   [0], y_line), fill=shadow_color, width=2)
                ...
                if character.unicode is True:
                    ...
                    if line_number == 1:
                        ...
                        draw.text(
                            (x, 25+((line_number-2)*7+(13*(line_number-3)))), character.character, font=fnt, fill=shadow_color)
                        draw.text(
                            (x, 25+((line_number-2)*7+(13*(line_number-3)))), character.character, font=fnt, fill=color)
                else:
                    if character.italics is True:
                        ...
                        if line_number == 1:
                            ...
                            img = draw_italics(
                                character.character, x+2, 25+((line_number-2)*7+(13*(line_number-3))), width, height, img, fnt, shadow_color)
                            img = draw_italics(
                                character.character, x, 23+((line_number-2)*7+(13*(line_number-3))), width, height, img, fnt, color)
                        ...
                        if line_number == 1:
                            ...
                            draw.text(
                                (x+2, 25+((line_number-2)*7+(13*(line_number-3)))), character.character, font=fnt, fill=shadow_color)
                            draw.text(
                                (x, 23+((line_number-2)*7+(13*(line_number-3)))), character.character, font=fnt, fill=color)

                # Draw Strike
                if character.strike is True:
                    if line_number == 1:
                        y_line = 14
                        draw.line((x, y_line, x + fnt.getsize(character.original)
                                   [0], y_line), fill=color, width=2)
                    else:
                ...
                if character.original == " ":
                    ...
                    size = fnt.getsize(character.original)[0]
                    x = size + x
            x = 8
...
"""
image, images = render(["§nMinecraft Formatting§r", "", "§00 §11 §22 §33",
                        "§44 §55 §66 §77", "§88 §99 §aa §bb", "§cc §dd §ee §ff", "§rk §kMinecraft", "§rl §lMinecraft", "§rm §mMinecraft", "§rn §nMinecraft", "§ro §oMinecraft", "§rlo §l§oMinecraft", "§rr §rMinecraft", "§rUnicode §r⚔❁☠☤❤✯"])
image.show()
if images == []:
    image.save("h.png")
else:
    for image1 in images:
        ...
        try:
            ...
            images.append(imageio.imread(img_byte_arr))
        except:
            pass
    imageio.mimsave('h.gif', images, fps=40)
"""
